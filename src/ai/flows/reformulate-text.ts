...
const reformulateTextFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      // Log the exact input being sent (first 500 chars for brevity)
      ...
      const {text: reformulatedTextResult} = await ai.generate({
        // model: 'googleai/gemini-1.5-flash-latest', // Optionally be explicit
        prompt: [{role: 'user', content: [{text: userPromptContent}]}], // User prompt
        systemInstruction: {role: 'system', content: [{text: systemPromptContent}]}, // System instruction
        output: {format: 'text'}, 
      ...
      // Log the problematic input for easier debugging
