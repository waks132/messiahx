...
const MAX_TEXT_LENGTH = 20000; // Increased limit, but still a safeguard
...
const reformulateTextFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    // Ensure reformulationPromptsConfig is defined and has the style
    const selectedPrompts = reformulationPromptsConfig && reformulationPromptsConfig[selectedStyleKey];

    if (!selectedPrompts || !selectedPrompts.system_prompt_template || !selectedPrompts.user_prompt_template) {
      console.error(`Reformulation style "${style}" not found or improperly configured in reformulationPromptsConfig. Available keys: ${reformulationPromptsConfig ? Object.keys(reformulationPromptsConfig).join(', ') : 'undefined'}`);
      return {
    ...
    try {
      const systemPromptContent = selectedPrompts.system_prompt_template; // Already includes the "detailed response" request.
      ...
      const {text: reformulatedTextResult} = await ai.generate({
        // model: 'googleai/gemini-1.5-flash-latest', // Optionally be explicit if default is problematic
        prompt: [{role: 'user', content: [{text: userPromptContent}]}],
        systemInstruction: {role: 'system', content: [{text: systemPromptContent}]}, // Using MessageData structure for systemInstruction
        output: {format: 'text'}, 
